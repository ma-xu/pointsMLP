...
def train(args, io):
    ...
    if (len(train_dataset) < 100):
        ...
        drop_last = True
    train_loader = DataLoader(train_dataset, num_workers=16, batch_size=args.batch_size, shuffle=True, drop_last=drop_last, pin_memory=True)
    test_loader = DataLoader(ShapeNetPart(partition='test', num_points=args.num_points, class_choice=args.class_choice),
                            num_workers=16, batch_size=args.test_batch_size, shuffle=False, drop_last=False, pin_memory=True)
