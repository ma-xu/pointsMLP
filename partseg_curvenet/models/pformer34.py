"""
Based on PointMLP 31, fix the std problem. from entire batch to channel-wise. Also move cls_token to previous stage.
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, heads=8, head_dim=64, activation='relu', similarity="dot", bias=True, res_expansion=1.0, **kwargs):
        """
...
def pformer34A(num_classes=50, **kwargs) -> Model:
...
def pformer34B(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="gelu", bias=True, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[1,1,1,1],
                 heads = [1,1,1,1], head_dims = [1,1,1,1], similarity="dot",
...
def pformer34C(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=32, groups=1, res_expansion=1.0,
...
def pformer34D(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="gelu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[1,1,1,1],
                 heads = [8,8,8,8], head_dims = [1,1,1,1], similarity="dot",
                 k_neighbors=[24, 24, 24, 7], reducers=[4, 4, 4, 4], de_dims=[512, 512, 256, 256],
                 de_blocks=[2,2,2,3], gmp_dim=16, **kwargs)

def pformer34E(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="gelu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2,2,2,2],
                 heads = [1,1,1,1], head_dims = [1,1,1,1], similarity="dot",
