"""
Based on pformer 35, add se layer
"""
...
class MultiConvBNReLURes1D(nn.Module):
    def __init__(self, channels, blocks=1, groups=1, res_expansion=1, bias=True, activation='relu'):
        """
        input[b,d,g]; output[b,d,g]
        :param channels:
        :param blocks:
        """
        super(MultiConvBNReLURes1D, self).__init__()
        operation = []
        for _ in range(blocks):
            operation.append(
                ConvBNReLURes1D(channels, groups=groups, res_expansion=res_expansion, bias=bias, activation=activation)
...
class PointNetFeaturePropagation(nn.Module):
    def __init__(self, in_channel, out_channel, blocks=1, groups=1, res_expansion=1.0, bias=True, activation='relu'):
        ...
        self.extraction = MultiConvBNReLURes1D(out_channel, blocks, groups=groups,
...
class Model(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        for i in range(len(de_blocks)):
            input_dim = de_dims[i]+en_dims[i+1]
            if i == len(de_blocks)-1:
                input_dim += (gmp_dim+16)
            self.decode_list.append(
                PointNetFeaturePropagation(input_dim, de_dims[i+1],
        ...
        self.gmp_map = nn.Sequential(
            ConvBNReLU1D(last_channel, last_channel//16, bias=bias, activation=activation),
            ConvBNReLU1D(last_channel//16, gmp_dim, bias=bias, activation=activation),

        )

        # classifier
        self.classifier = nn.Sequential(
            nn.Conv1d(de_dims[-1], 256, 1, bias=bias),
            nn.BatchNorm1d(256),
            self.act,
            SELayer(256),
            nn.Dropout(0.5),
    ...
    def forward(self, x, cls_label):
        ...
        global_context = F.adaptive_max_pool1d(self.gmp_map(x), 1)  # [b, dim, 1]
        #here is the cls_token
        cls_token =cls_label.unsqueeze(dim=-1)  # [b, 16, 1]
        ...
        for i in range(len(self.decode_list)):
            if i == len(self.decode_list)-1:
                 x = torch.cat([x, global_context.expand([-1, -1, x.shape[-1]]), cls_token.expand([-1, -1, x.shape[-1]])], dim=1)
...
def pformer37A(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 ...
                 k_neighbors=[24, 24, 24, 7], reducers=[4, 4, 4, 4], de_dims=[512, 512, 256, 256],
                 de_blocks=[2,2,2,3], gmp_dim=16, **kwargs)

def pformer37B(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[1,1,1,1],
                   heads = [6,6,6,6], head_dims = [64,64,64,64], similarity="dot",
...
def pformer37C(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2,2,2,2],
                   heads = [6,6,6,6], head_dims = [64,64,64,64], similarity="dot",
...
def pformer37D(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
...
def pformer37E(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=0.25,
                 activation="gelu", bias=True, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[1,1,1,1],
                   heads = [6,6,6,6], head_dims = [64,64,64,64], similarity="dot",
                 k_neighbors=[24, 24, 24, 7], reducers=[4, 4, 4, 4], de_dims=[512, 512, 256, 256],
...
def pformer37F(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[1,1,1,1],
                   heads = [6,6,6,6], head_dims = [64,64,64,64], similarity="dot",
                 k_neighbors=[32, 32, 31, 7], reducers=[4, 4, 4, 4], de_dims=[512, 512, 256, 256],
                 de_blocks=[2,2,2,3], gmp_dim=16, **kwargs)


def pformer37G(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=0.25,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[1,1,1,1],
                   heads = [6,6,6,6], head_dims = [64,64,64,64], similarity="dot",
...
def pformer37H(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=0.25,
                 activation="relu", bias=True, use_xyz=False, normalize="center",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[1,1,1,1],
                   heads = [6,6,6,6], head_dims = [64,64,64,64], similarity="dot",
                 k_neighbors=[32, 32, 31, 7], reducers=[4, 4, 4, 4], de_dims=[512, 512, 256, 256],
                 de_blocks=[2,2,2,3], gmp_dim=16, **kwargs)

def pformer37I(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=32, groups=1, res_expansion=0.5,
                 activation="relu", bias=True, use_xyz=False, normalize="center",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[1,1,1,1],
                   heads = [6,6,6,6], head_dims = [64,64,64,64], similarity="dot",
                 k_neighbors=[32, 32, 31, 7], reducers=[4, 4, 4, 4], de_dims=[512, 512, 256, 256],
...
def pformer37J(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=32, groups=1, res_expansion=0.5,
                 activation="gelu", bias=True, use_xyz=False, normalize="center",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[3,3,3,3], pos_blocks=[2,2,2,2],
                   heads = [6,6,6,6], head_dims = [64,64,64,64], similarity="dot",
                 k_neighbors=[32, 32, 31, 7], reducers=[4, 4, 4, 4], de_dims=[512, 512, 256, 256],
                 de_blocks=[2,2,2,3], gmp_dim=16, **kwargs)

def pformer37K(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=32, groups=1, res_expansion=0.5,
