"""
Based on pformer 34, add parameters for x + para*trans(x)
"""
...
class ConvBNReLURes1D(nn.Module):
    def __init__(self, channel, kernel_size=1, groups=1, res_expansion=1.0, bias=True, activation='relu', init_values=1e-4, **kwargs):
        ...
        self.ls1 = nn.Parameter(init_values * torch.ones(1,channel,1))

    def forward(self, x):
        return self.act(self.ls1*self.net2(self.net1(x)) + x)
...
def pformer36A(num_classes=50, **kwargs) -> Model:
...
def pformer36B(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[1,1,1,1],
                   heads = [6,6,6,6], head_dims = [64,64,64,64], similarity="dot",
...
def pformer36C(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2,2,2,2],
                   heads = [6,6,6,6], head_dims = [64,64,64,64], similarity="dot",
...
def pformer36D(num_classes=50, **kwargs) -> Model:
    return Model(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
