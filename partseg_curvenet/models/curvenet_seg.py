...
class CurveNet(nn.Module):
    ...
    def forward(self, xyz, l=None):
        ...
        print(f"l0_points.shape is {l0_points.shape}")

        l1_xyz, l1_points = self.cic11(xyz, l0_points)
        print(f"l1_xyz, l1_points = self.cic11(xyz, l0_points) {l1_xyz.shape} {l1_points.shape}")
        l1_xyz, l1_points = self.cic12(l1_xyz, l1_points)
        print(f"l1_xyz, l1_points = self.cic12(l1_xyz, l1_points) {l1_xyz.shape} {l1_points.shape}")

        l2_xyz, l2_points = self.cic21(l1_xyz, l1_points)
        print(f"l2_xyz, l2_points = self.cic21(l1_xyz, l1_points) {l2_xyz.shape} {l2_points.shape}")
        l2_xyz, l2_points = self.cic22(l2_xyz, l2_points)
        print(f"l2_xyz, l2_points = self.cic22(l1_xyz, l1_points) {l2_xyz.shape} {l2_points.shape}")

        l3_xyz, l3_points = self.cic31(l2_xyz, l2_points)
        print(f"l3_xyz, l3_points = self.cic31(l2_xyz, l2_points) {l3_xyz.shape} {l3_points.shape}")
        l3_xyz, l3_points = self.cic32(l3_xyz, l3_points)
        print(f"l3_xyz, l3_points = self.cic32(l3_xyz, l3_points) {l3_xyz.shape} {l3_points.shape}")
 
        l4_xyz, l4_points = self.cic41(l3_xyz, l3_points)
        print(f"l4_xyz, l4_points = self.cic41(l3_xyz, l3_points) {l4_xyz.shape} {l4_points.shape}")
        l4_xyz, l4_points = self.cic42(l4_xyz, l4_points)
        print(f"l4_xyz, l4_points = self.cic42(l4_xyz, l4_points) {l4_xyz.shape} {l4_points.shape}")

        l5_xyz, l5_points = self.cic51(l4_xyz, l4_points)
        print(f"l5_xyz, l5_points = self.cic51(l4_xyz, l4_points) {l5_xyz.shape} {l5_points.shape}")
        l5_xyz, l5_points = self.cic52(l5_xyz, l5_points)
        print(f"l5_xyz, l5_points = self.cic52(l5_xyz, l5_points) {l5_xyz.shape} {l5_points.shape}")
        l5_xyz, l5_points = self.cic53(l5_xyz, l5_points)
        print(f"l5_xyz, l5_points = self.cic53(l5_xyz, l5_points) {l5_xyz.shape} {l5_points.shape}")

        # global features
        emb1 = self.global_conv1(l4_points)
        print(f"emb1 = self.global_conv1(l4_points) {emb1.shape}")
        ...
        print(f"l4_points = self.fp4(l4_xyz, l5_xyz, l4_points, l5_points) {l4_points.shape}")
        l4_xyz, l4_points = self.up_cic5(l4_xyz, l4_points)
        print(f"l4_xyz, l4_points = self.up_cic5(l4_xyz, l4_points) {l4_xyz.shape} {l4_points.shape}")

        l3_points = self.fp3(l3_xyz, l4_xyz, l3_points, l4_points)
        print(f"l3_points = self.fp3(l3_xyz, l4_xyz, l3_points, l4_points) {l3_points.shape}")
        l3_xyz, l3_points = self.up_cic4(l3_xyz, l3_points)
        print(f"l3_xyz, l3_points = self.up_cic4(l3_xyz, l3_points) {l3_xyz.shape} {l3_points.shape}")

        l2_points = self.fp2(l2_xyz, l3_xyz, l2_points, l3_points)
        print(f"l2_points = self.fp2(l2_xyz, l3_xyz, l2_points, l3_points) {l2_points.shape}")
        l2_xyz, l2_points = self.up_cic3(l2_xyz, l2_points)
        print(f"l2_xyz, l2_points = self.up_cic3(l2_xyz, l2_points) {l2_xyz.shape}")

        l1_points = self.fp1(l1_xyz, l2_xyz, l1_points, l2_points)
        print(f"l1_points = self.fp1(l1_xyz, l2_xyz, l1_points, l2_points) {l1_points.shape}")

        if l is not None:
            l = l.view(batch_size, -1, 1)
            emb = torch.cat((emb1, emb2, l), dim=1) # bs, 128 + 16, 1
        l = emb.expand(-1,-1, xyz.size(-1))
        print(f"l = emb.expand(-1,-1, xyz.size(-1)) {l.shape}")
        x = torch.cat((l1_xyz, l1_points, l), dim=1)
        print(f"x = torch.cat((l1_xyz, l1_points, l), dim=1) {x.shape}")

        xyz, x = self.up_cic2(l1_xyz, x)
        print(f"xyz, x = self.up_cic2(l1_xyz, x) {xyz.shape} {x.shape}")
        xyz, x = self.up_cic1(xyz, x)
        print(f"xyz, x = self.up_cic1(xyz, x) {xyz.shape} {x.shape}")
        ...
        x = self.drop1(x)
        x = self.conv2(x)
        print(f"x = self.conv2(x) {x.shape}")
        return x
