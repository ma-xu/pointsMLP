...
class PointNetNoBN(nn.Module):
    def __init__(self, num_classes=40, use_normals=False, **kwargs):
        super(PointNetNoBN, self).__init__()
        if use_normals:
            channel = 6
        else:
            channel = 3
...
if __name__ == '__main__':
    ...
    model = PointNetNoBN(k=40, use_normals=True)
    out = model(data)
    print(out.shape)

    print("===> testing pointNet without use_normals")
    data = torch.rand(10, 3, 1024)
    model = PointNetNoBN(k=40, use_normals=False)
    out = model(data)
    print(out.shape)
