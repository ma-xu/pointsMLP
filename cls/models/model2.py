"""
Based on model1, only add dropout layers to Transformers, nothing else.
"""
import torch
...
class ContextAttention(nn.Module):
    def __init__(self, dim, inner_dim, group=1, pool="max", dropout=0.):
        super().__init__()
        ...
        self.to_out = nn.Sequential(
            nn.Conv2d(inner_dim, dim, 1, groups=group, bias=False),
            nn.Dropout(dropout)
        )
...
class Transformer(nn.Module):
    def __init__(self, dim, inner_dim, group=1, pool="max", dropout=0., **kwargs):
        super(Transformer, self).__init__()
        self.norm = nn.LayerNorm(dim)
        self.attention = ContextAttention(dim=dim, inner_dim=inner_dim, group=group, pool=pool, dropout=dropout)
...
class LocalExtraction(nn.Module):
    def __init__(self, dim, inner_dim, group=1, pool="max", blocks=3, dropout=0., **kwargs):
        super(LocalExtraction, self).__init__()
        operation = []
        for _ in range(blocks):
            operation.append(
                Transformer(dim, inner_dim, group, pool, dropout)
            )
...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, down=4, dropout=0.):
        super().__init__()
        inner_dim = dim // down
        self.heads = heads
...
class TransformerGlobal(nn.Module):
    def __init__(self, dim, heads=8, down=2, dropout=0., **kwargs):
        super(TransformerGlobal, self).__init__()
        self.norm = nn.LayerNorm(dim)
        self.attention = Attention(dim, heads, down,dropout)
        self.ffn = nn.Sequential(
            nn.LayerNorm(dim),
            nn.Linear(dim, dim),
            nn.LeakyReLU(inplace=True),
...
class Model2(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, pool="max",
                 ...
                 local_blocks=[2, 2, 2, 2], global_blocks=3, dropout=0.1,
        ...
        for i in range(len(local_blocks)):
            ...
            Local_extraction = LocalExtraction(last_channel, last_channel * self.expansion,
                                               self.groups, self.pool, local_block_num, dropout)
            self.local_blocks_list.append(Local_extraction)
            last_channel = last_channel * 2

        global_blocks_list = []
        for _ in range(global_blocks):
            transformer_global = TransformerGlobal(last_channel, self.groups, down=2, dropout=dropout)
...
def model2A1(num_classes=40, **kwargs) -> Model2:  # 77.307 neu
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="max",
                 k_neighbors=[32, 32], expansion=2, groups=8,
                 local_blocks=[2, 2], global_blocks=3, dropout=0.1,
                 reducers=[4, 4], **kwargs)

def model2A2(num_classes=40, **kwargs) -> Model2:  # 75.434 neu
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="max",
                 k_neighbors=[32, 32], expansion=2, groups=8,
...
def model2A3(num_classes=40, **kwargs) -> Model2:  # 77.307  neu
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="max",
...
def model2A4(num_classes=40, **kwargs) -> Model2:  # 77.273 neu
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="max",
...
def model2A5(num_classes=40, **kwargs) -> Model2:  # 75.503 neu
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="max",
...
def model2A6(num_classes=40, **kwargs) -> Model2:  # 76.475 neu
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="max",
                 k_neighbors=[32, 32], expansion=2, groups=8,
                 local_blocks=[2, 2], global_blocks=3, dropout=0.1,
                 reducers=[8, 4], **kwargs)
...
def model2A7(num_classes=40, **kwargs) -> Model2:  # 76.856 neu
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="max",
                 k_neighbors=[32, 32], expansion=2, groups=8,
...
def model2A8(num_classes=40, **kwargs) -> Model2:  # 77.03 neu
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="max",
                 k_neighbors=[32, 32], expansion=2, groups=8,
                 local_blocks=[2, 2], global_blocks=2, dropout=0.1,
                 reducers=[4, 4], **kwargs)

def model2A9(num_classes=40, **kwargs) -> Model2:  # 75.33% neu, not finished, epoch107
    return Model2(points=1024, class_num=num_classes, embed_dim=128, pool="max",
                 k_neighbors=[32, 32], expansion=2, groups=8,
                 local_blocks=[2, 2], global_blocks=3, dropout=0.1,
                 reducers=[4, 4], **kwargs)


def model2A10(num_classes=40, **kwargs) -> Model2:  # 77.065, neu
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="max",
                 k_neighbors=[32, 16], expansion=2, groups=8,
                 local_blocks=[2, 2], global_blocks=3, dropout=0.1,
                 reducers=[8, 4], **kwargs)


def model2A11(num_classes=40, **kwargs) -> Model2:  # 74.219  neu
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="max",
                 k_neighbors=[16, 16], expansion=2, groups=8,
                 local_blocks=[2, 2], global_blocks=3, dropout=0.1,
                 reducers=[4, 4], **kwargs)


def model2A12(num_classes=40, **kwargs) -> Model2:  # 75.954 neu
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="max",
                 k_neighbors=[24, 24], expansion=2, groups=8,
                 local_blocks=[2, 2], global_blocks=3, dropout=0.1,
                 reducers=[4, 4], **kwargs)
...
def model2B(num_classes=40, **kwargs) -> Model2:
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="mean",
                 k_neighbors=[32, 32], expansion=2, groups=8,
                 local_blocks=[2, 2], global_blocks=3,
                 reducers=[4, 4], **kwargs)

def model2C(num_classes=40, **kwargs) -> Model2:
    return Model2(points=1024, class_num=num_classes, embed_dim=64, pool="logsumexp",
                 k_neighbors=[32, 32], expansion=2, groups=8,
                 local_blocks=[2, 2], global_blocks=3,
                 reducers=[4, 4], **kwargs)

def model2D(num_classes=40, **kwargs) -> Model2:
    return Model2(points=1024, class_num=num_classes, embed_dim=128, pool="max",
                 k_neighbors=[32, 32], expansion=2, groups=8,
                 local_blocks=[2, 2], global_blocks=3,
                 reducers=[4, 4], **kwargs)

def model2E(num_classes=40, **kwargs) -> Model2:
    return Model2(points=1024, class_num=num_classes, embed_dim=128, pool="max",
                 k_neighbors=[64, 64], expansion=2, groups=8,
                 local_blocks=[2, 2], global_blocks=3,
                 reducers=[4, 4], **kwargs)

def model2F(num_classes=40, **kwargs) -> Model2:
    return Model2(points=1024, class_num=num_classes, embed_dim=128, pool="max",
                 k_neighbors=[32, 32], expansion=2, groups=8,
...
if __name__ == '__main__':
    ...
    print("===> testing model2A1-12")
    for model in [model2A1(),model2A2(),model2A3(),model2A4(),model2A5(),model2A6(),model2A7(),
                  model2A8(),model2A9(),model2A10(),model2A11(),model2A2()]:
        out = model(data)
        print(out.shape)
