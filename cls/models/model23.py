"""
Based on model22, more efficient random implementation, mlp attention
...
class Attention(nn.Module):
    def __init__(self, dim, neighbors):
        super().__init__()
        self.param = nn.Parameter(torch.rand(1,dim,1,neighbors))
    def forward(self, x):  # [b,d,p]
        x = x.unsqueeze(dim=-1)
        x = (x*self.param).sum(dim=-1,keepdim=False)
        return x


class TransformerBlock(nn.Module):
    def __init__(self, dim, neighbors, **kwargs):
        """
        ...
        self.attention = Attention(dim=dim, neighbors=neighbors)
...
class PreExtraction(nn.Module):
    def __init__(self, channels, neighbors, blocks=1):
        """
        ...
        self.transformer = TransformerBlock(channels, neighbors)
...
class Model23(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64,
        ...
        for i in range(len(pre_blocks)):
            ...
            pre_block_module = PreExtraction(out_channel, kneighbor, pre_block_num)
            self.pre_blocks_list.append(pre_block_module)
            # append pos_block_list
            pos_block_module = PosExtraction(out_channel, anchor_points, pos_block_num)
...
def model23A(num_classes=40, **kwargs) -> Model23:
    return Model23(points=1024, class_num=num_classes, embed_dim=64,
                   pre_blocks=[2, 2, 2], pos_blocks=[2, 2, 2], k_neighbors=[32, 32, 32],
                   reducers=[4, 2, 2], **kwargs)


def model23B(num_classes=40, **kwargs) -> Model23:
    return Model23(points=1024, class_num=num_classes, embed_dim=32,
                   pre_blocks=[2, 2, 2], pos_blocks=[2, 2, 2], k_neighbors=[32, 32, 32],
                   reducers=[4, 2, 2], **kwargs)
