...
class PointMLP9(nn.Module):
    def __init__(self, num_classes=13, c=9, points=4096, embed_dim=128, normal_channel=True,
                 pre_blocks=[2,2,2,2], pos_blocks=[2,2,2,2], k_neighbors=[32,32,32,32],
                 reducers=[2,2,2,2], **kwargs):
        super(PointMLP9, self).__init__()
        ...
        self.encoder_stage2 = encoder_stage(anchor_points=points//16, channel=256, reduce=True,
                                            pre_blocks=4, pos_blocks=4, k_neighbor=32)
        self.encoder_stage3 = encoder_stage(anchor_points=points // 64, channel=256, reduce=False,
                                            pre_blocks=4, pos_blocks=4, k_neighbor=32)
        self.encoder_stage4 = encoder_stage(anchor_points=points // 128, channel=512, reduce=True,
                                            pre_blocks=4, pos_blocks=4, k_neighbor=32)
        ...
        self.fp1 = PointNetFeaturePropagation(in_channel=256+embed_dim, out_channel=256, blocks=4 )
    ...
    def forward(self, x): # [b,n,c]
        ...
        l0_points = self.fp1(xyz, xyz_1, x, l1_points)
