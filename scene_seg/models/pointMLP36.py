...
class LocalGrouper(nn.Module):
    def __init__(self, channel, groups, kneighbors, use_xyz=True, normalize="anchor", **kwargs):
        ...
        if self.normalize is not None:
            # add_channel=10 if self.use_xyz else 0
    ...
    def forward(self, xyz, points):
        ...
        if self.normalize is not None:
            ...
            if self.normalize =="anchor":
                mean = new_points.unsqueeze(dim=-2)  # [B, npoint, 1, d+10]
                std = torch.std(grouped_points-mean)
            grouped_points = (grouped_points-mean)/(std + 1e-5)
            grouped_points = self.affine_alpha*grouped_points + self.affine_beta
        if self.use_xyz:
            # here, we use the augmented xyz, that is cat[norm, x-xi, x, xi]
            relative = grouped_xyz- new_xyz.unsqueeze(-2).expand_as(grouped_xyz)
            relative_norm = torch.norm(relative, p="fro", dim=-1, keepdim=True)
            augmented = torch.cat([relative_norm, relative, new_xyz.unsqueeze(-2).expand_as(grouped_xyz), grouped_xyz],
                                  dim=-1)
            grouped_points = torch.cat([grouped_points, augmented], dim=-1)  # [B, npoint, k, d+10]

        new_points = torch.cat([grouped_points, new_points.unsqueeze(dim=-2).expand(-1, -1, self.kneighbors, -1)], dim=-1)
...
class PreExtraction(nn.Module):
    def __init__(self, channels, out_channels,  blocks=1, groups=1, res_expansion=1, bias=True,
        ...
        in_channels = 10+2*channels if use_xyz else 2*channels
...
class PointMLP36(nn.Module):
    def __init__(self, num_classes=13,c=9, points=4096, embed_dim=64, groups=1, res_expansion=1.0,
                 ...
                 **kwargs):
        super(PointMLP36, self).__init__()
        self.stages = len(pre_blocks)
        self.class_num = num_classes
        self.points = points
        self.embedding = ConvBNReLU1D(c, embed_dim, bias=bias, activation=activation)
...
def model36A(num_classes=13, **kwargs) -> PointMLP36:
    return PointMLP36(num_classes=num_classes, c=9, points=4096, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
...
def model36B(num_classes=13, **kwargs) -> PointMLP36:
    return PointMLP36(num_classes=num_classes, c=9, points=4096, embed_dim=32, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 128, 128], de_blocks=[4,4,4,4],
...
def model36C(num_classes=13, **kwargs) -> PointMLP36:
    return PointMLP36(num_classes=num_classes, c=9, points=4096, embed_dim=32, groups=1, res_expansion=1.0,
...
def model36D(num_classes=13, **kwargs) -> PointMLP36:
    return PointMLP36(num_classes=num_classes, c=9, points=4096, embed_dim=32, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 128, 64], de_blocks=[3,3,3,3],
                 **kwargs)
