...
class PointMLP31(nn.Module):
    def __init__(self, num_classes=50, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 ...
                 de_dims=[512, 256, 128, 128], de_blocks=[2, 2, 2, 2], **kwargs):
...
def model31A(num_classes=13, **kwargs) -> PointMLP31:
    return PointMLP31(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                      ...
                      dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                      k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
                      de_dims=[512, 256, 128, 128], de_blocks=[4, 4, 4, 4], **kwargs)


def model31B(num_classes=31, **kwargs) -> PointMLP31:
    return PointMLP31(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                      activation="relu", bias=True, use_xyz=False, normalize="anchor",
                      dim_expansion=[2, 2, 2, 2], pre_blocks=[3, 3, 3, 3], pos_blocks=[3, 3, 3, 3],
                      k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
                      de_dims=[512, 256, 128, 128], de_blocks=[3, 3, 3, 3], **kwargs)
...
def model31E(num_classes=13, **kwargs) -> PointMLP31:
    return PointMLP31(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                      ...
                      k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
                      de_dims=[512, 256, 128, 128], de_blocks=[6, 6, 6, 6], **kwargs)
...
if __name__ == '__main__':
    ...
    model = model31G(13)
    out = model(data)  # [2,2048,13]
    print(out.shape)

    # model = model31A(13)
    # xyz = torch.rand(6, 9, 2048)
    # out,_ = model(xyz)
    # print(out.shape)  # [batch, 2048, 13]
