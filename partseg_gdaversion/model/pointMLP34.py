"""
Based on PointMLP33, change to 2D conv (effect the BN)
Based on PointMLP31, change the head.
...
class PreExtraction(nn.Module):
    def __init__(self, channels, out_channels,  blocks=1, groups=1, res_expansion=1, bias=True,
        ...
        self.transfer = ConvBNReLU2D(in_channels, out_channels, bias=bias, activation=activation)
        operation = []
        for _ in range(blocks):
            operation.append(
                ConvBNReLURes2D(out_channels, groups=groups, res_expansion=res_expansion,
                                bias=bias, activation=activation)
            )
        self.operation = nn.Sequential(*operation)
        self.pool = nn.AdaptiveAvgPool2d((None, 1))

    def forward(self, x):  # [batch, npoints, kneighbors, dim]
        x = x.permute(0, 3, 1, 2)  # [b, d, n, k]
        x = self.transfer(x)
        x = self.operation(x)  # [b, d, n, k]
...
class PointMLP34(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 ...
                 gmp_dim=64, have_se=True, **kwargs):
        super(PointMLP34, self).__init__()
        ...
        for en_dim in en_dims[:2]:  # we only consider the last two stage outputs.
            self.gmp_map_list.append(ConvBNReLU1D(en_dim, gmp_dim, bias=bias, activation=activation))

        # classifier
        self.classifier_1 = nn.Sequential(
            nn.Conv1d(gmp_dim+gmp_dim+16+3+de_dims[-1], 256, 1, bias=bias),
            nn.BatchNorm1d(256),
            get_activation(activation)
        )
        self.have_se = have_se
        if self.have_se:
            self.se=nn.Sequential(nn.AdaptiveAvgPool1d(1),
                                nn.Conv1d(256, 256//8, 1, bias=bias),
                                nn.BatchNorm1d(256//8),
                                get_activation(activation),
                                nn.Conv1d(256//8, 256, 1, bias=bias),
                                nn.Sigmoid())
    ...
    def forward(self, x, norm_plt, cls_label):
        ...
        for i in range(2): # only last two stages
            gmp_list.append(F.adaptive_max_pool1d(self.gmp_map_list[i](x_list[i]), 1))  # b, 64, 1

        #here is the cls_token
        cls_label = cls_label.unsqueeze(dim=-1)  # b, 16, 1
        x = torch.cat([x, (gmp_list[0]).expand(-1, -1, x.shape[-1]), (gmp_list[1]).expand(-1, -1, x.shape[-1]),
                       cls_label.expand(-1, -1, x.shape[-1]), xyz_list[-1].permute(0, 2, 1)], dim=1)
...
def model34G1(num_classes=50, **kwargs) -> PointMLP34:
    return PointMLP34(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
...
def model34G2(num_classes=50, **kwargs) -> PointMLP34:
    return PointMLP34(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
...
def model34G3(num_classes=50, **kwargs) -> PointMLP34:
    return PointMLP34(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 31, 7], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 128, 128], de_blocks=[4,4,4,4],
                 gmp_dim=64, have_se=True, **kwargs)


def model34G4(num_classes=50, **kwargs) -> PointMLP34:
    return PointMLP34(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
...
def model34G5(num_classes=50, **kwargs) -> PointMLP34:
    return PointMLP34(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
...
def model34G6(num_classes=50, **kwargs) -> PointMLP34:
    return PointMLP34(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[24, 24, 24, 4], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 128, 128], de_blocks=[4,4,4,4],
                 gmp_dim=64, have_se=True, **kwargs)

def model34G7(num_classes=50, **kwargs) -> PointMLP34:
    return PointMLP34(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
...
def model34G8(num_classes=50, **kwargs) -> PointMLP34:
    return PointMLP34(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 128, 128], de_blocks=[4,4,4,4],
                 gmp_dim=64, have_se=False, **kwargs)
