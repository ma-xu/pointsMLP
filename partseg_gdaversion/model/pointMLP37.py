...
class LocalGrouper(nn.Module):
    def __init__(self, channel, groups, kneighbors, use_xyz=True, normalize="anchor", select="random", **kwargs):
        """
    ...
    def forward(self, xyz, points):
        ...
        if self.select=="fps":
            ...
            fps_idx = torch.cat([torch.randperm(N)[:S].unsqueeze(0) for _ in range(B)],dim=0).to(xyz.device).long()
...
class PointMLP37(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        for i in range(len(pre_blocks)):
            ...
            local_grouper = LocalGrouper(last_channel, anchor_points, kneighbor, use_xyz, normalize, select=select)  # [b,g,k,d]
...
def model37A(num_classes=50, **kwargs) -> PointMLP37:
    return PointMLP37(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=0.25,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor", select="fps",
                 dim_expansion=[2, 2, 2, 1], pre_blocks=[1, 1, 2, 1], pos_blocks=[1, 1, 2, 1],
                 k_neighbors=[24, 24, 24, 6], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 128, 128], de_blocks=[2,2,2,2],
                 gmp_dim=64,cls_dim=64, **kwargs)

def model37B(num_classes=50, **kwargs) -> PointMLP37:
    return PointMLP37(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=0.25,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor", select="random",
                 dim_expansion=[2, 2, 2, 1], pre_blocks=[1, 1, 2, 1], pos_blocks=[1, 1, 2, 1],
                 k_neighbors=[24, 24, 24, 6], reducers=[4, 4, 4, 4],
...
def model37C(num_classes=50, **kwargs) -> PointMLP37:
    return PointMLP37(num_classes=num_classes, points=2048, embed_dim=32, groups=1, res_expansion=0.25,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor", select="random",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[1, 1, 1, 1],
                 k_neighbors=[24, 24, 24, 6], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 256, 128], de_blocks=[2,2,2,2],
                 gmp_dim=64,cls_dim=64, **kwargs)

def model37D(num_classes=50, **kwargs) -> PointMLP37:
    return PointMLP37(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=0.25,
                 activation="leakyrelu0.2", bias=True, use_xyz=False, normalize="anchor", select="random",
                 dim_expansion=[2, 2, 2, 1], pre_blocks=[2, 2, 2, 2], pos_blocks=[1, 1, 1, 1],
                 k_neighbors=[24, 24, 24, 12], reducers=[4, 4, 4, 2],
                 de_dims=[512, 256, 256, 128], de_blocks=[2,2,2,2],
                 gmp_dim=64,cls_dim=64, **kwargs)
