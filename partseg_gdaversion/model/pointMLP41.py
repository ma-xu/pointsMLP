"""
FIx the bug of std calculation: Local region wise
...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        if self.normalize is not None:
            ...
            # local region std
            b_n, g_n, _ =  new_xyz.shape
            std = torch.std((grouped_points-mean).reshape(b_n,g_n,-1),dim=-1,keepdim=True).unsqueeze(dim=-1)
...
class PointMLP41(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        super(PointMLP41, self).__init__()
...
def model41A(num_classes=50, **kwargs) -> PointMLP41:  # 0.85881 0.85861
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
...
def model41B(num_classes=50, **kwargs) -> PointMLP41: # 0.85836  0.85864
    return PointMLP41(num_classes=num_classes,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
...
def model41C(num_classes=50, **kwargs) -> PointMLP41: # 0.85707
    return PointMLP41(num_classes=num_classes,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
...
def model41D(num_classes=50, **kwargs) -> PointMLP41: # 0.85560
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=32, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 128, 128], de_blocks=[4,4,4,4],
...
def model41E(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
...
def model41F(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
...
# model41G_cos 0.85927 model41G_cos-lr0.001new0.85824 model41G_cos-lr0.0050.859973
# model41G_cos_lr0.01 0.85831 model41G_cos-noinit 0.85904
def model41G(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
...
def model41GA(num_classes=50, **kwargs) -> PointMLP41:  # 0.85825
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
...
def model41GB(num_classes=50, **kwargs) -> PointMLP41: # 0.85860
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
...
def model41GC(num_classes=50, **kwargs) -> PointMLP41: # 0.86015
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
...
def model41G1(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
...
def model41G2(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[24, 24, 24, 24], reducers=[4, 4, 4, 4],
...
def model41G3(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
...
def model41H(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="gelu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
...
def model41I(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
...
def model41X1(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
...
def model41X2(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=128, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
...
def model41X3(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
...
def model41X4(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=0.125,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 128, 128], de_blocks=[4,4,4,4],
...
def model41X5(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="gelu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
...
def model41X6(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="leakyrelu0.2", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
...
def model41X7(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="leakyrelu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
...
def model41X8(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=128, groups=8, res_expansion=0.25,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
...
def model41K(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 24, 7], reducers=[4, 4, 4, 4],
...
def model41L(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
...
def model41M(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=False, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
...
def model41N(num_classes=50, **kwargs) -> PointMLP41:
    return PointMLP41(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 24, 6], reducers=[4, 4, 4, 4],
